...
Both numbers reproduce exactly when re-running the configs as-written. **Caveat:** the 30-epoch baseline is undertrained — its slow cosine schedule plus patience-5 early stopping cuts it off at epoch 12, and a clean run on a faster (15-epoch) schedule reaches ~78% with no augmentation. On a matched 15-epoch schedule, augmentation still adds a verified **+1.86 ± 0.57 pts** test accuracy (3 seeds, paired; 78.2% → 80.0%, best single run 80.8%), so the physics-augmentation effect is real but smaller than the headline +9.8 implies — most of that gap was the training schedule. The macro-F1 gain (+0.010 ± 0.011) is within noise, so the robust claim is on accuracy. See `EXPERIMENTS.md` for the backbone sweep (`training/sweep_backbones.py`, EfficientNet-B0 wins), the training-regime reconciliation, and the 3-seed verification (`training/verify_seeds.py`).
